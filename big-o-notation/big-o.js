...
function uniqueLinear(words) {
  ...
  for (
    ...
    const word = words[i];          // 2 * n = 2(n) ~= 0(n)
    if (!seen[word]) {              // 3 * n = 3(n) ~= 0(n)
      seen[word] = true;            // 2 * n = 2(n) ~= 0(n)
      unique[unique.length] = word; // 3 * n = 3(n) ~= 0(n)
    }
  }
...
function uniqueQuadratic(words) {
  ...
  for (
    ...
    if (isUnique) {                 // 1 * n = O(n)
      unique[unique.length] = word; // 3 * n = 0(3n) ~= 0(n)
    }
  }
  return unique;                    // 1 * 1 = O(1)
